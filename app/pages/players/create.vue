...
<template>
  <div class="max-w-2xl mx-auto py-6">
    <div class="mb-6">
      <h1 class="text-2xl font-semibold mb-2">{{ t.players.createTitle }}</h1>
      <p class="text-gray-600">{{ t.players.createSubtitle }}</p>
    </div>
    ...
    >
      <template #description>
        <div class="space-y-2">
          <p>{{ t.players.credentialsInfo }}</p>
          <div class="bg-gray-50 p-3 rounded-md font-mono text-sm">
            <p><strong>{{ t.players.emailLabel }}:</strong> {{ generatedCredentials.email }}</p>
            <p><strong>{{ t.players.passwordLabel }}:</strong> {{ generatedCredentials.password }}</p>
          </div>
          <div v-if="autoRegistrationCount > 0" class="mt-3 p-3 bg-blue-50 rounded-md">
            <div class="flex items-start gap-2">
              ...
              <div class="text-sm text-blue-700">
                <p class="font-medium">{{ t.players.autoRegistered }}</p>
                <p>{{ tr(t.players.autoRegisteredDesc, { count: autoRegistrationCount }) }}</p>
              </div>
            </div>
          </div>
          <p class="text-xs text-gray-600">{{ t.players.shareCredentials }}</p>
        </div>
      </template>
    </UAlert>
    ...
    <UCard>
      <form @submit.prevent="createPlayer" class="space-y-6">
        ...
        <UFormGroup
          :label="t.players.emailLabel"
          :hint="form.email.trim() ? t.players.emailHintCustom : t.players.emailHintAuto"
        >
          <UInput
            v-model="form.email"
            type="email"
            :placeholder="generatedEmail || t.players.emailPlaceholder"
            :disabled="loading"
        ...
        <div v-if="finalEmail" class="bg-gray-50 p-3 rounded-md">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-600">{{ t.players.finalEmail }}:</span>
        ...
        <UFormGroup :label="t.players.phoneNumber" :hint="t.common.optional">
          <UInput
            ...
            type="tel"
            :placeholder="t.players.phoneNumberPlaceholder"
            :disabled="loading"
          />
        </UFormGroup>
        ...
        <div class="bg-blue-50 p-4 rounded-lg">
          <div class="flex items-start gap-3">
            ...
            <div class="text-sm">
              <p class="font-medium text-blue-900">{{ t.players.autoSetupTitle }}</p>
              <div class="text-blue-700 mt-1 space-y-1">
                <p>• {{ t.players.autoSetupEmail }}</p>
                <p>• {{ t.players.autoSetupPassword }}</p>
                <p>• {{ t.players.autoSetupEvents }}</p>
                <p>• {{ t.players.autoSetupCredentials }}</p>
              </div>
            </div>
          </div>
        </div>
